...
class TestsState(rx.State):
    ...
    def generate_test(self) -> str:
        ...
        return test_text  # Возврат компонента rx.text с сгенерированным текстом
...
def test_details():
    return rx.cond(  # Условный рендеринг
        ...
        rx.vstack(  # Вертикальное расположение компонентов
            ...
            rx.text(TestsState.generate_test()),  # Вызов функции для генерации представления теста
        ),
...
def add_test_form():
    return rx.vstack(  # Вертикальное расположение компонентов
        ...
        rx.text_area(  # Многострочное поле ввода для JSON данных теста
            ...
            on_change=TestsState.handle_json_data_change,  # Обработчик изменения
            value=TestsState.new_test_json_data_as_string,  # Значение поля ввода
        ),
