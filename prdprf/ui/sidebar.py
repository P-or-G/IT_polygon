    ...
    return rx.box(
        rx.hstack(
            ...
            rx.text(rx.color_mode_cond(
                light="Включить тёмный режим",
                dark="Включить светлый режим",
            ),
                size="4"),
            width="100%",
            padding_x="0.5rem",
...
def sidebar_items() -> rx.Component:
    return rx.vstack(
        sidebar_item("Дэшборд", "layout-dashboard", navigation.routes.HOME_ROUTE),
        sidebar_item("Уроки", "globe", navigation.routes.ARTICLE_LIST_ROUTE),
        sidebar_item("Ваши уроки", "newspaper", navigation.routes.BLOG_POSTS_ROUTE),
        sidebar_item("Создать урок", "notebook-pen", navigation.routes.BLOG_POST_ADD_ROUTE),
        sidebar_item("Обратная связь", "mail", navigation.routes.CONTACT_US_ROUTE),
        sidebar_item("Обращения", "mailbox", navigation.routes.CONTACT_ENTRIES_ROUTE),
        spacing="1",
        width="100%",
    )
