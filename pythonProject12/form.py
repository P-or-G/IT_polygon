...
from prdprf.lessons.state import LessonPostState
...
class CommentState(rx.State):
    post_id = LessonPostState.blog_post_id()
    ...
    def load_comments(self):
        ...
        with rx.session() as session:
            self.comments = session.query(Comment).filter(Comment.post_id == self.post_id).all()
    ...
    def add_comment(self):
        ...
        if self.comment_content.strip():
            new_comment = Comment(
                ...
                post_id=self.post_id,  # Используем глобальную переменную
            )
            with rx.session() as session:
